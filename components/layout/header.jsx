...
export default function Header() {
  ...
  // Update the getMainCategories function to use consistent URLs
  const getMainCategories = () => {
    ...
    allCategories.forEach((categoryName) => {
      ...
      if (!grouped[groupName]) {
        grouped[groupName] = {
          href: `/products?category=${encodeURIComponent(groupName)}`,
          subcategories: [],
        }
      }
  ...
  // Update the category groups for mobile menu to use main categories
  const categoryGroups = {
    "Living Room": "/products?category=Living Room",
    Dining: "/products?category=Dining",
  ...
  // Modified navigation links: removed Buyer Gallery from main nav
  const navLinks = [
    { name: "Home", href: "/" },
  ...
  return (
    <div className="fixed w-full z-50 flex justify-center mt-3 px-4">
      <header
        className={`w-full max-w-[1440px] rounded-full transition-all duration-300 ${isScrolled ? "bg-white shadow-md" : "bg-white/70 backdrop-blur-md"
          } py-2 px-4`}
      >
            ...
            {navLinks.map((link) =>
              link.hasDropdown ? (
                ...
                >
                  <Link
                    href={link.href}
                    className={`flex items-center text-base font-medium hover:text-amber-500 transition-colors ${pathname.startsWith(link.href) ? "text-amber-500" : ""
                      } ${link.isItalic ? "italic" : ""}`}
                  >
                    {link.name}
                    <ChevronDown
        ...
        >
          <div className="bg-white/90 backdrop-blur-sm rounded-lg shadow-xl border border-gray-200/50 w-[1000px] p-6">
                    ...
                    {Object.entries(mainCategories).map(([categoryName, categoryData]) => (
                      <li key={categoryName}>
                        <Link
                          ...
                          className={`block px-3 py-2 rounded-md text-sm font-medium transition-colors ${hoveredCategory === categoryName
            ...
            <div className="mt-6 pt-4 border-t border-gray-200/50 flex justify-between items-center">
              ...
              {/* Add Buyer Gallery link here */}
              <Link
                href="/buyer-gallery"
                className="inline-block px-6 py-2 bg-amber-50/80 text-amber-600 rounded-md text-sm font-medium hover:bg-amber-100/80 transition-colors italic"
              >
                Buyer Gallery
              </Link>
            </div>
      ...
      {/* Mobile menu */}
      <div
        className={`fixed inset-0 bg-white z-50 transition-transform duration-300 ${isMenuOpen ? "translate-x-0" : "translate-x-full"
          }`}
      >
        ...
        <div className="overflow-y-auto h-[calc(100vh-56px)]">
          ...
          <nav className="p-4">
            <ul className="space-y-3">
                ...
                <li key={link.name}>
                  {link.hasDropdown ? (
                    <div>
                      <Link
                        href={link.href}
                        className={`block py-2 text-sm font-bold ${pathname === link.href || pathname.startsWith(link.href) ? "text-amber-500" : ""
                          }`}
                        onClick={closeMenu}
                      >
                        {link.name}
                      ...
                      <div className="ml-4 mt-2 space-y-2">
                        {Object.entries(mainCategories).map(([categoryName, categoryData]) => (
                          <div key={categoryName} className="space-y-1">
                            <Link
                              href={categoryData.href}
                              className="block py-1 text-sm font-medium text-gray-700 hover:text-amber-500"
                              onClick={closeMenu}
                            >
                              {categoryName}
                            </Link>
                            {/* Subcategories with thin font */}
                              ...
                              <div className="ml-4 space-y-1">
                                {categoryData.subcategories.slice(0, 5).map((subcategory) => (
                                  <Link
                                    key={subcategory.name}
                                    href={subcategory.href}
                                    className="block py-0.5 text-xs font-light text-gray-600 hover:text-amber-400"
                                    onClick={closeMenu}
                                  >
                                    {subcategory.name}
                                  </Link>
                    ...
                    <Link
                      href={link.href}
                      className={`block py-2 text-sm font-bold ${pathname === link.href ? "text-amber-500" : ""
                        }`}
                      onClick={closeMenu}
                    >
                      {link.name}
              ...
              {/* Add Buyer Gallery with different color */}
              <li className="pt-3 border-t">
                <Link
                  href="/buyer-gallery"
                  className="block py-2 text-sm font-bold text-purple-600 italic"
                  onClick={closeMenu}
                >
                  Buyer Gallery
                </Link>
              ...
              <li className="pt-3">
                <Link
                  href="/cart"
                  className="flex items-center py-2 text-sm font-bold"
                  onClick={closeMenu}
                >
                ...
                {user ? (
                  <div className="space-y-2">
                    ...
                    {isAdmin && (
                      <Link
                        href="/admin"
                        className="flex items-center py-2 text-sm font-bold text-amber-600"
                        onClick={closeMenu}
                      >
                    ...
                    <button
                      ...
                      className="flex items-center py-2 text-sm font-bold text-red-600 w-full"
                    >
                      Sign Out
                    </button>
                  </div>
